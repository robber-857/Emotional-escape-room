...
| 关卡 | 实际内容 | 工程重点 |
| --- | --- | --- |
| L1 分离之河 | 修桥、游泳、泳圈、船；对岸人物和灯 | 河边→对岸镜头连续；路径互斥、人物/灯动作顺序、未点亮灯分支 |
| L2 失联房间 | 座位与钥匙、半开门、耳环、家具摆放 | 每局四组、首把钥匙必失败、15 秒寻找、自由摆放判定 |
| L3 风暴大厅 | 门、等待、右窗帘、左窗、风扇；出口物品 | 风暴状态、拒绝分支可通关、可选动作一次计分、只携带一件 |
| L4 远方之门 | 微光门、静塔门、森林门、宫殿门 | 无新谜题、出口互斥、最终确认与幂等结算 |
...
“门能否通行”与“半开的门行为评分”分离。建议主厅通往 L3 的必需通道与耳环支线入口分开；若玩家拒绝进入半开门，仍可在主厅选择其他任务。耳环组随后提供重新进入探索或明确跳过该组的入口，家具组始终可进入，拒绝一扇门不能卡死整关。

“所有四组都提供”不等于强迫玩家对每组选择“是”。四组均需完成，或达到规则允许的明确拒绝/跳过终态后才能进入 L3；没有显式跳过定义的组，不能由计时自动跳过。耳环未进入情况下的跳过是否映射到 PDF 已有拒绝分值尚无依据，暂记录独立 skip/not_observed，不伪造“不寻找”得分。这条新通行与跳过规则需要在首阶段冻结。
...
家具通过 SVG 空间内拖动或“选家具→点位置→确认”两种方式操作。保存归一化 x/y、角度、尺寸和房间模板版本，不保存客户端 CSS 像素。后端验证边界、重叠限制与摆放分类。只在确认摆放时结算，不按 pointermove 计分。
